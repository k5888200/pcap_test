...
int main(int argc, char* argv[]) {
  ...
  while (true) {
	...
	printf("dst MAC: ");
	for(int i=0;i<6;i++) printf("%02X%c",eh->ether_dhost[i], i<5?':':'\n');
	printf("src MAC: ");
	for(int i=0;i<6;i++) printf("%02X%c",eh->ether_shost[i], i<5?':':'\n');
	...
	if(ntohs(eh->ether_type) == ETH_P_IP){
		...
		if(iph->protocol == IPPROTO_TCP){
			...
			if(payload_size){
				puts("------ data -----");
				for(int i=0;i<std::min(payload_size, 16);i++) printf("%02x ",payload[i]); printf("\n");
			}
		}
		else if(iph->protocol == IPPROTO_ICMP){
			puts("- Internet Control Message Protocol(ICMP)");
		}
	...
	else if(eh->ether_type == ETH_P_ARP){
		puts("- ARP");
	}
	else if(eh->ether_type == ETH_P_IPV6){
		puts("- IPv6");
	}
	else {
